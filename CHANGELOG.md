...
### Fixed
- **dev.sh install command**: Now properly removes existing symlinks before installing
  - Previously failed with "identical files" error when trying to install over a symlink
  - `./dev.sh install` now correctly replaces symlinks with regular file installations
  - Makes transitioning from development mode to production install seamless
